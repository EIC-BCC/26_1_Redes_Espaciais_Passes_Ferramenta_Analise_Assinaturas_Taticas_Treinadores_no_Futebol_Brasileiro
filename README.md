...
[PDF do TCC](TCC_Avaliacao_Treinador.pdf)
...
Com o notebook aberto, execute as células em sequência. Algumas partes do código estão comentadas por não serem necessárias executarem novamente. O restante pode ser executado célula por célula, na ordem em que aparece.
...
```mermaid
graph TD;
    A[Dados das partidas] --> B[Arquivos PKL];
    B --> C[Criação dos DataFrames];
    C --> D[Tratamento dos eventos e passes];
    D --> E[Conversão e padronização das coordenadas];
    E --> F[Divisão do campo em zonas];
    F --> G[Construção da rede espacial];
    G --> H[Cálculo das métricas];
    H --> I[Identificação exploratória dos estilos espaciais];
    I --> J[Geração dos gráficos e visualizações];
    J --> K[Análise dos resultados por equipe e treinador];
```
